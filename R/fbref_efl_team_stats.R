#' Get EFL Championship team statistics from FBref
#'
#' This function retrieves EFL Championship team statistics from FBref for a
#' specific season and type.
#'
#' @name fbref_efl_team_stats
#' @param season A character string representing the season in the
#' format 'YYYY/YYYY'.
#' @param type A character string specifying the type of statistics to retrieve.
#'   Valid types are: 'standard', 'goalkeeping', 'advanced_goalkeeping',
#'   'shooting', 'passing',
#'   'pass_types', 'goal_creation', 'defensive_actions', 'possession',
#'   'playing_time', or 'miscellaneous'.
#'
#' @return A data frame containing the requested team statistics.
#'
#' @details The function uses the 'rvest' package to scrape data from FBref.
#' The 'season' parameter should be in a valid 'YYYY/YYYY' format.
#'
#'
...
fbref_efl_team_stats <- function(season = NULL, type = NULL) {

  code <- 10
  league_stats <- 'Championship-Stats'
