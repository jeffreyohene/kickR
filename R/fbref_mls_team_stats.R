#' Retrieve Major League Soccer (MLS) team statistics from FBref.
#'
#' This function scrapes team statistics for Major League Soccer (MLS) from
#' FBref for a specified season and type.
#'
#' @name fbref_mls_team_stats
#'
#' @param season The season for which you want to retrieve statistics.
#' Should be in the format 'YYYY' (e.g., '2023').
#'               If not provided, the current season will be used.
#'
#' @param type The type of statistics to retrieve. Available types include
#'              'standard', 'goalkeeping', 'advanced_goalkeeping',
#'             'shooting', 'passing', 'pass_types', 'goal_creation',
#'             'defensive_actions', 'possession', 'playing_time',
#'             and 'miscellaneous'. If not provided, 'standard' statistics will
#'             be retrieved.
#'
...
fbref_mls_team_stats <- function(season = NULL, type = NULL) {
  base_url <- 'https://fbref.com/en/comps/'
  code <- 22
  league_stats <- 'Major-League-Soccer-Stats'


  valid_seasons <- c('2018', '2019', '2020', '2021', '2022', '2023')
  ...
  check_season <- function(season = NULL) {
    ...
    if (grepl(pattern, normalized_season)) {
      formatted_season <- normalized_season
    } else {
      message('Invalid season format. The season must be in a valid format.')
      return(NULL)
    }
    ...
    if (formatted_season %in% valid_seasons) {
      ...
      message('Data unavailable for this year.')
      return(NULL)
    }
  }
